...
const List = () => {
  ...
  useEffect(() => {
    ...
    if (
      selectedDate[0].startDate.getTime() === selectedDate[0].endDate.getTime()
    ) {
    ...
    try {
      ...
      const filteredResults = response.filter(
        (item) =>
          ...
          item.timeshares.some(
            (timeshare) =>
              timeshare.price >= minPrice && timeshare.price <= maxPrice
          )
      );

      const timeshares = filteredResults.reduce((acc, realestate) => {
        const realestateTimeshares = realestate.timeshares.filter(
          (timeshare) => {
            const timeshareStart = new Date(timeshare.startDay);
            const timeshareEnd = new Date(timeshare.endDay);
            const selectedStart = new Date(date[0].startDate);
