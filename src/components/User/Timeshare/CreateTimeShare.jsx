...
function CreateTimeShare({ onCreateSuccess, realestateId: initialRealestateId }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(BASE_URL + "API/TimeShares/CreateTimeShare", timeShareData);
            Swal.fire({
                icon: "success",
                ...
                <DialogContent className="dialog-content">
                    <form onSubmit={handleSubmit} className="form">
                        <Typography variant="h6" className="form-field">
                            ...
                            />
                            {errors.startDay && <span className="error-message">{errors.startDay}</span>}
                        </Typography>
                        <Typography variant="h6" className="form-field">
                            <label htmlFor="endDay" className="form-label">
                                End Day:
                            </label>
                            <input type="date" value={endDay} onChange={(e) => handleEndDayChange(e.target.value)} />
                            {errors.endDay && <span className="error-message">{errors.endDay}</span>}
                        </Typography>
                        <Typography variant="h6" className="form-field">
